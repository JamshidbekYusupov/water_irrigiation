...
log_path = r'C:\Irrigation_Water_Requirement\logging\tuning.log'
logging.basicConfig(
    filename=log_path,
    filemode='a',
    level=logging.INFO,
...
logging.info(f'Tuning has started')
...
class Tuning_Pipeline(BaseEstimator):
    ...
    def tune_grid_search(self, param_grid):
        try:
            ...
            # Update the algorithm and model
            self.algorithm = grid_search.best_params_
            self.model = grid_search.best_estimator_
            
            logging.info(f"Grid Search complete. Best params: {self.algorithm}")
            return self
        except Exception as e:
...
# Saving model to the models folder
    ...
    def getting_metrics(self):
        try:
            ...
            out_path = os.path.join(metrics_path, 'Improved_results.txt')
